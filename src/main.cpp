...
uint256 hashGenesisBlock("0x51a1c5a323c58130e45acf21fbd9442b8810f06d8b0d04f20ad711dd837a214c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // W2Coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "W2Coin Signed Message:\n";
...
{
    // W2Coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 168 * 60 * 60;
static const int64 nTargetSpacing = 1 * 600;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // W2Coin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
   ...
   /* current difficulty formula, gravity well */
        ...
        if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
                ...
                for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                        ...
                        if (LatestBlockTime < BlockReading->GetBlockTime()) {
                                if (BlockReading->nHeight > 1000) LatestBlockTime = BlockReading->GetBlockTime();
                        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // W2Coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // W2Coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x51a1c5a323c58130e45acf21fbd9442b8810f06d8b0d04f20ad711dd837a214c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1399843508;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084895071;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // W2Coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("wwwcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("wwwcoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running W2Coin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("wwwcoinMiner terminated\n");
        throw;
    }
}
